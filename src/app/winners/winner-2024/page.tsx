import { winner2024Image, winner2024Sections } from './winner2024Data';

export default function Winner2024Page() {
  return (
    <main className="winners-page winners-2024-page">
      <section className="winners-hero">
        <p className="winners-kicker">Winners 2024</p>
        <h1>CIO Power List 2024</h1>
        <p>
          All 2024 winner members are grouped below as cards with a shared default image for a
          consistent presentation.
        </p>
      </section>

      {winner2024Sections.map((section) => (
            ...
            {section.entries.map((winner) => (
              <article key={`${section.title}-${winner.name}`} className="winner-profile-card">
                <div className="winner-profile-media">
                  <img src={winner2024Image} alt={winner.name} className="winner-profile-image" />
                </div>
