import { winner2016Image, winner2016Sections } from './winner2016Data';

export default function Winner2016Page() {
  return (
    <main className="winners-page winners-2016-page">
      <section className="winners-hero">
        <p className="winners-kicker">Winners 2016</p>
        <h1>CIO Power List 2016</h1>
        <p>
          All 2016 winner members are grouped below as cards with a shared default image for a
          consistent presentation.
        </p>
      </section>

      {winner2016Sections.map((section) => (
            ...
            {section.entries.map((winner) => (
              <article key={`${section.title}-${winner.name}`} className="winner-profile-card">
                <div className="winner-profile-media">
                  <img src={winner2016Image} alt={winner.name} className="winner-profile-image" />
                </div>
