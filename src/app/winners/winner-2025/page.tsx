import { winner2025Image, winner2025Sections } from './winner2025Data';

export default function Winner2025Page() {
  return (
    <main className="winners-page winners-2025-page">
      <section className="winners-hero">
        <p className="winners-kicker">Winners 2025</p>
        <h1>CIO Power List 2025</h1>
        <p>
          All 2025 winner members are grouped below as cards with a shared default image for a
          consistent presentation.
        </p>
      </section>

      {winner2025Sections.map((section) => (
            ...
            {section.entries.map((winner) => (
              <article key={`${section.title}-${winner.name}`} className="winner-profile-card">
                <div className="winner-profile-media">
                  <img src={winner2025Image} alt={winner.name} className="winner-profile-image" />
                </div>
