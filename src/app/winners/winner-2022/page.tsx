import { winner2022Image, winner2022Sections } from './winner2022Data';

export default function Winner2022Page() {
  return (
    <main className="winners-page winners-2022-page">
      <section className="winners-hero">
        <p className="winners-kicker">Winners 2022</p>
        <h1>CIO Power List 2022</h1>
        <p>
          All 2022 winner members are grouped below as cards with a shared default image for a
          consistent presentation.
        </p>
      </section>

      {winner2022Sections.map((section) => (
            ...
            {section.entries.map((winner) => (
              <article key={`${section.title}-${winner.name}`} className="winner-profile-card">
                <div className="winner-profile-media">
                  <img src={winner2022Image} alt={winner.name} className="winner-profile-image" />
                </div>
