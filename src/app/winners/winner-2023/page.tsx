import { winner2023Image, winner2023Sections } from './winner2023Data';

export default function Winner2023Page() {
  return (
    <main className="winners-page winners-2023-page">
      <section className="winners-hero">
        <p className="winners-kicker">Winners 2023</p>
        <h1>CIO Power List 2023</h1>
        <p>
          All 2023 winner members are grouped below as cards with a shared default image for a
          consistent presentation.
        </p>
      </section>

      {winner2023Sections.map((section) => (
            ...
            {section.entries.map((winner) => (
              <article key={`${section.title}-${winner.name}`} className="winner-profile-card">
                <div className="winner-profile-media">
                  <img src={winner2023Image} alt={winner.name} className="winner-profile-image" />
                </div>
